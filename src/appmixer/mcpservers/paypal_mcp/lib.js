...
const SERVER_SCRIPT_PATH = __dirname + '/node_modules/@paypal/mcp/dist/index.js';
...
module.exports = {
    ...
    mcpCall: async (context, env, method, args) => {
        ...
        processArgs.push('--tools=all');
        processArgs.push(`--access-token="${env.PAYPAL_ACCESS_TOKEN}"`);
        processArgs.push(`--paypal-environment="${env.PAYPAL_ENVIRONMENT}"`);
